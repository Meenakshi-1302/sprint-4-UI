...
// import { sendOtp, verifyOtp } from '../../services/ThirdPartyCredentials/ThirdPartyCredentialsForgetService';
...
//       const response = await verifyOtp(otpwithoutCommas,email);
...
const ClientOtp = () => {
  ...
  const email = useSelector((state) => state.email.email); // Fetching email from Redux
  ...
  const handleResend = async () => {
    if (!email) {
      toast.error('No email stored to send OTP. Please try again.');
      return;
    }
    
    setIsLoading(true);
    try {
      await sendOtp(email); // Pass the email from Redux here
      toast.success('OTP has been resent to your email.');
      setOtp(['', '', '', '', '', '']);
      setTimer(60);
      setIsResendEnabled(false);
